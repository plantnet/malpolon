...
from malpolon.data.environmental_raster import PatchExtractor, RasterTorchGeo

DATA_PATH = Path("malpolon/tests/")

def test_patch_query_torchgeo():
    class Sentinel2(RasterTorchGeo):
        filename_glob = DATA_PATH / "torchgeo_sentinel2_test.tif"
        is_image = True
        separate_files = True
        all_bands = ["B08"]
        rgb_bands = ["B08"]
    class MicroLifeClef(RasterTorchGeo):
    ...
    patch1_s2 = dataset_s2[{'lon':3.87075, 'lat':43.61135, 'crs':dataset_s2.crs_pyproj.geodetic_crs, 'units':'pixel', 'size': (100, 100)}]['image'][0]
    patch2_s2 = dataset_s2[{'lon':570265.8337376957, 'lat':4829076.115471331, 'crs':dataset_s2.crs_pyproj, 'units':'m', 'size': 1000}]['image'][0]
    patch_mlc = dataset_mlc[{'lon':3.87075, 'lat':43.61135, 'crs':dataset_mlc.crs_pyproj, 'units':'m', 'size': (930*0.7142857142857143*100, 930*100)}]['image'][0]
