...
def extract_sample_from_raster(
    ...
    (width, height) in rasterio's Window, this method extracts a
    ...
    with rasterio.open(input_path) as src:
        ...
        profile = src.profile
        profile.update(width=sample_data.shape[2], height=sample_data.shape[1])  # sample_data.shape -> (1, rows (y), cols (x))
...
def main() -> None:
    """Run main script."""
    input_path = '/home/tlarcher/Documents/Pl@ntNet/git/malpolon/examples/sentinel2_raster_torchgeo/dataset/T31TEJ_20190801T104031_B02_10m.tif'
    output_path = 'torchgeo_sentinel2_sample.tif'
    size = (200, 200)

    extract_sample_from_raster(input_path, output_path, size)
    compare_rasters(input_path, output_path, size)
