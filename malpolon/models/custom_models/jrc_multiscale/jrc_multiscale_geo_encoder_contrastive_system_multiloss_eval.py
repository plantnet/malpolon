        ...
        for epoch_counter in range(self.args.last_epoch, self.args.epochs + self.args.last_epoch):
            ...
            for step, train_dict in enumerate(tqdm(train_loader)):
                ...
                if step % self.args.log_every_n_steps_train == 0:       
                    ...
                    log_similarity_matrix_step(sim_matrices, epoch_counter, step, n_modalities=len(modalities_to_process), log_images=self.log_images, mode='train')
            ...
            for top1, top5, modality_name in zip(top1s, top5s, modalities_name):
                wandb.log({f"acc_epoch (batch avg)/train/top1_{modality_name}": top1,
                           f"acc_epoch (batch avg)/train/top5_{modality_name}": top5})
