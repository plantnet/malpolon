...
class RasterTorchGeoDataset(RasterDataset):
    ...
    def _load_observation_data(
        ...
        if any([root is None, obs_fn is None]):
            return pd.DataFrame(columns=['lon', 'lat', 'speciesId', 'split'])
        ...
        try:
            ...
            ind = df.index[df["split"].isin(subsets)]
            df = df.loc[ind]
    ...
    def _valid_query_point(self,
        ...
        if query['crs'] != epsg4326:
            transformer = Transformer.from_crs(query['crs'], epsg4326, always_xy=True)
    ...
    def __getitem__(
        ...
        if not isinstance(query, BoundingBox):
            ...
            return sample, label, query_obs_id
