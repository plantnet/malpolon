...
def main(val_ratio: float = 0.05):
    pa_train = pd.read_csv('GCL24-PA-metadata-train_belgium.csv')
    ...
    pa_train.to_csv('GLC24-PA-metadata-train_belgium_without_val-5%.csv', index=False)
    pa_val.to_csv('GLC24-PA-metadata_belgium-val-5%.csv', index=False)
    pa_train_val = pd.concat([pa_train, pa_val])
    pa_train_val.to_csv('GLC24-PA-metadata-train_val_belgium-5%_split_by_species_frequency.csv', index=False)
