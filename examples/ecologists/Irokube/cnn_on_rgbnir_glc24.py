...
from malpolon.data.datasets.geolifeclef2024 import (
    MultipleRasterPatchProvider, PatchesDataset, PatchesDatasetMultiLabel)
...
class Sentinel2PatchesDataModule(BaseDataModule):
    ...
    def get_dataset(self, split, transform, target_transform=None, **kwargs):
        mrp_rgbnir = MultipleRasterPatchProvider(self.dataset_path + 'sentinel-2a/belgium_crop/',
                                                 select=[# 's2_red_2021_crop_belgium',
                                                         # 's2_green_2021_crop_belgium',
                                                         # 's2_blue_2021_crop_belgium',
                                                         #'s2_nir_2021_crop_belgium',
                                                         #'s2_swir1_2021_crop_belgium',
                                                         's2_swir2_2021_crop_belgium'],
                                                 size=self.size)  # 'dataset/jpeg_patches_sample_stats_bidon.csv')
        if 'multiclass' in self.task:
            dataset = PatchesDataset(
                occurrences=self.dataset_path + self.labels_name,
                providers=[mrp_rgbnir],
                transform=transform,
                target_transform=target_transform,
                id_name='surveyId',
                labels_name='speciesId',
                item_columns=['lat', 'lon', 'surveyId', 'speciesId', 'split'],
                **kwargs
            )
        else:
...
@hydra.main(version_base="1.3", config_path="config", config_name="cnn_on_rgbnir_glc24.yaml")
