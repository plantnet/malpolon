...
def main(cfg: DictConfig) -> None:
    ...
    classif_model = ClassificationSystem(cfg.model, **cfg.optimizer, **cfg.task)
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            dirpath=log_dir,
            filename="checkpoint-{epoch:02d}-{step}-{" + f"{next(iter(classif_model.metrics.keys()))}/val" + ":.4f}",
            monitor=f"{next(iter(classif_model.metrics.keys()))}/val",
            mode="max",
        ),
    ]
    ...
    if cfg.run.predict_type == 'test_dataset':
        ...
        query_point = {'observation_id': test_data.observation_ids[0],
                       ...
                       'species_id': [test_data[0][1].astype(int)]}  # Adjust value in case of multilabel classification
