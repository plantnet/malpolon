...
from malpolon.data.datasets.jrc_multiscale import (
    LandscapeDatasetSimple, load_LUCAS_img, SpeciesDatasetSimple, load_species_img,
)
...
METADATA_PATHS = {
    ...
    'test':  os.path.join(ROOT_PATH_SPECIES, "gps_noisy/glc24_pa_test_private_CBN-med_matching-LUCAS-500m_merged_with_species_noise_mixture.csv"),
                 }
...
class SpeciesGPSErrorDetection(SpeciesDatasetSimple):
    def __init__(
        self,
        root_path: str = None,
        ...
        **kwargs
        ...
        super().__init__(root_path, fp_metadata, transform, dataset_kwargs, query_id, **kwargs)
        self.verbose = verbose
    
    def load_species_img_error_detection(
        self,
        ids: Union[str, int],
        img_dir: str,
        ...
        imgs = []
        missing_imgs = []
        for speciesKey in ids:
            img, fp = load_species_img(int(speciesKey), img_dir, img_formats=['jpeg', 'jpg', 'png'], return_img_path=True)
            if img.sum() == 0:
                missing_imgs.append(int(speciesKey))
                if self.verbose:
                        print(f'[WARNING]: Image of speciesKey {speciesKey} not found. Skipping...')
            else:
                imgs.append(img)

        res = [imgs]
        if return_ids:
            res.append([int(id) for id in ids])
        res.append(missing_imgs)
        return tuple(res)

    def __getitem__(self, index) -> Any:
        print(f'Sample {index}')
        imgs, gps = [], self.coords
        
        if not self.metadata.empty:
            sample = self.metadata.iloc[index]
            imgs, speciesKeys, missing_imgs = self.load_species_img_error_detection(
                sample['speciesKeys'].split(),
                self.root_path,
                return_ids=True,
                **self.dataset_kwargs
            )
            imgs = [self.transform(img) for img in imgs]  # Transform is not optional since images need to be of equal shape to be stacked
        ...
        return imgs, torch.Tensor(gps), torch.Tensor(gps_noisy), torch.tensor(gps_match), torch.tensor([surveyId]), torch.tensor([speciesKeys]), n_imgs, missing_imgs
...
def collate_species(original_batch):
    imgs, gpss, gpss_noisy, gps_match, surveyIds, speciesKeys, n_imgs, missing_imgs = zip(*original_batch)
    ...
    surveyIds_batched = torch.stack(list(surveyIds), dim=0)
    
    # Quantities with variable amounts (impossible to stack)
    speciesKeys_batched = list(speciesKeys)

    # No stacking needed
    n_imgs_batched = np.array(n_imgs).reshape(-1, 1)
    missing_imgs_batched = np.concatenate(list(missing_imgs), axis=0)
    return img_batched, gps_batched, gpss_noisy_batched, gps_match_batched, surveyIds_batched, speciesKeys_batched, n_imgs_batched, missing_imgs_batched
...
model = model_species.to(args.device)  # Must happen before instanciating he optimizer in case of loading a checkpoint
...
if args.ckpt_path:
    ...
    species_sd = filter_state_dict_keys_data_parallel(checkpoint['state_dict'], prefix='species.')
    model.load_state_dict(species_sd)
...
def run_inference(
    ...
    with torch.no_grad():
        for step, data in tqdm(enumerate(test_loader), total=len(test_loader)):
            print(f'Batch {step}')
            images, gps, gps_noisy, gps_match, surveyIds, speciesKeys, n_img_per_ids, missing_imgs = data
            if len(missing_imgs) > 0:
                print(f"[Batch {step}] Missing Species images: {len(missing_imgs)}")
            ...
            # Forward pass
            """
            There are 1 to k species image per geolocalized sample.
            """
...
for seed in tqdm(SEEDS, 'Test set seeds'):
    ...
    dataset_test = SpeciesGPSErrorDetection(
