...
def main(args):
    ...
    if args.arch == 'species':
        ...
        test_dataset = SatelliteDatasetSimple(
            ...
            fp_metadata = 'dataset/scale_3_satellite/glc24_pa_test-private_CBN-med_matching-LUCAS-500m.csv',
        ...
        test_dataset = MultiscaleDatasetSimple(
            ...
            fp_metadata_species = None,
            root_path_landscape = 'dataset/scale_2_landscape/',
            fp_metadata_landscape = None,
            root_path_satellite = 'dataset/scale_3_satellite/PA_Train_SatellitePatches/',
            fp_metadata_satellite = 'dataset/scale_3_satellite/glc24_pa_test-private_CBN-med_matching-LUCAS-500m.csv',
    ...
    # Evaluation strategy
    if args.eval_type == 'linear_probing':
        classifier = MultiLabelClassifier(model['satellite'].modality_encoder, model['satellite'].gps_encoder, embed_dim, num_labels, type)
    elif args.eval_type == 'fine_tuning':
        classifier = MultiLabelClassifier(model, out_dim=args.out_dim, dropout=args.dropout)
    ...
    args.log_every_n_steps_test = min(args.log_every_n_steps_train, len(test_loader))

    # Run
    ## It’s a no-op if the 'gpu_index' argument is a negative integer or None.
    with torch.cuda.device(args.gpu_index):
        simclr = SimCLR(model=model, optimizer=optimizer, scheduler=cosine_scheduler, args=args)
        simclr.train(test_loader, test_loader, max_iter=args.max_iter)
