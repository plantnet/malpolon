...
EUNIS_LVL = ['2', '3_4']  # List of values in [1, 2, 3, 3_4]
...
BETAS = (0.9, 0.999)
EPS = 1e-8
LABEL_SMOOTHING = 0.1  # Float in [0, 1]
DROPOUT_LORA = 0.1
NUM_WORKERS = 4
DROP_PATH = 0.2
...
# Model
# ----------------------------
model = get_model(MODEL, 10, drop_path_rate=DROP_PATH).to(DEVICE)  # Doesn't mater what number of classes we put here since we are going to replace the head with a custom one adapted for multilabels. This is just to re-use code written to instantiate models.
...
#     bias="none",
#     modules_to_save=["heads.1", "heads.2", "heads.3", "heads.4", "heads.3_4", ]
# )
...
cosine_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
    optimizer,
    T_max=EPOCHS,  # exclude warmup period
    eta_min=1e-6
)
...
def run_epoch(loader, split, epoch_nb, optimizer, training=True):
    ...
    for (images,
        ...
        if training:
            loss.backward()
            scheduler = get_lr_scheduler(epoch)
            scheduler.step()
...
if not INFERENCE:
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            model.save_pretrained(f"{MODEL}_lora_best")
        ...
        model.save_pretrained(f"{MODEL}_lora_last")
