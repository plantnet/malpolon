...
def get_multihead_model(model_name, num_classes):
    match model_name:
        case 'resnet18':
            print("[INFO] Using ResNet18")
            model = models.resnet18(weights="IMAGENET1K_V1")
...
class MultiHeadModel(nn.Module):
    def __init__(self, backbone, bb_model_name, feature_dim, n_classes1, n_classes2, n_classes3):
        super().__init__()

        self.backbone = backbone
        self.bb_model_name = bb_model_name

        self.head1 = nn.Linear(feature_dim, n_classes1)
        self.head2 = nn.Linear(feature_dim, n_classes2)
        self.head3 = nn.Linear(feature_dim, n_classes3)
    ...
    def extract_features(self, x):
        ...
        if hasattr(self.backbone, "forward_features"):
            ...
            if isinstance(feat, dict):
                ...
                # feat = torch.cat([feat["x_norm_clstoken"], feat["x_norm_regtokens"].mean(dim=1)], dim=-1)  # don't forget  to adjust head input size x2
            else:
