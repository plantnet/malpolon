...
if MODEL == 'resnet18':
    ...
    model_specific_transforms = [# transforms.Resize((224, 224)),
                                #  transforms.Resize(250),  # gap between resize and centercrop is to remove copyright watermark on images
                                #  transforms.CenterCrop((224, 224)),
                                 transforms.Resize(540),  # gap between resize and centercrop is to remove copyright watermark on images
...
model = MultiHeadModel(model, MODEL, EUNIS_LVL,
                       ...
                       gradcam_head=EUNIS_LVL[0]).to(DEVICE)

def reshape_transform(tensor):
    # tensor shape: (B, seq_len, embed_dim)
    # seq_len = 1 (CLS) + 4 (register tokens) + num_patches
    num_patches_side = int(np.sqrt(tensor.shape[1] - 5))  # subtract CLS + 4 register tokens
    result = tensor[:, 5:, :]                              # drop CLS and register tokens
    ...
    for (images,
        ...
        if split == 'test':
            ...
            for i in range(len(images)):
                pred = outputs[i].argmax(axis=-1).item()
                targets = [ClassifierOutputTarget(pred)]
                grayscale_cam = cam(
                    input_tensor=images[i].unsqueeze(0),
                    targets=targets,#labels_enc_ohs['3_4'][0:1],
                )[0]
                ...
                visualization = show_cam_on_image(
                    minmax_scale(torch.permute(images[i], (1, 2, 0))).detach().cpu().numpy(),
                    grayscale_cam,
                    use_rgb=True
                )
                from PIL import Image, ImageDraw, ImageFont
                img_gradcam = Image.fromarray(visualization.astype(np.uint8))
                img_original = Image.fromarray((255*minmax_scale(torch.permute(images[i], (1, 2, 0)))).detach().cpu().numpy().astype(np.uint8))
                def add_class_on_img(img, labels_enc_oh_lvl3_4, pred, loader, i):
                    draw = ImageDraw.Draw(img)
                    labels = ''
                    for k, v in enumerate(labels_enc_oh_lvl3_4[i]):
                        if v == 1:
                            labels += loader.dataset.label_encoding_table['3_4'][k] + ';'
                    draw.text(
                        xy=(10, 10),        # (x, y) position from top-left
                        text=f"Label: {labels}, pred: {loader.dataset.label_encoding_table['3_4'][pred]}",
                        fill=(255, 255, 255),   # RGB color, red here
                        font=ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", size=20),
                    )

                add_class_on_img(img_gradcam, labels_enc_ohs['3_4'], pred, loader, i)
                add_class_on_img(img_original, labels_enc_ohs['3_4'], pred, loader, i)
                img_gradcam.save(os.path.join(OUTPUT_DIR, 'inference/', f'gradcam_{i}.png'))
                img_original.save(os.path.join(OUTPUT_DIR, 'inference/', f'original_{i}.png'))
                

        # loss = get_criterion(outputs, labels_enc_ohs)
        loss = 0

        for lvl, weight in zip(EUNIS_LVL, EUNIS_LVL_WEIGHTS):
            loss += get_criterion(outputs, labels_enc_ohs[lvl])
...
if MULTILABEL_CORRESPONDANCE_STRATEGY in ['soft_ml', 'ml']:
    ...
    for lvl in EUNIS_LVL:
        ...
        np.save(f'{Path(TEST_METRICS).parent}/confusion_matrix_eunis-{lvl}', test_metrics[1])
        np.save(f'{Path(TEST_METRICS).parent}/confusion_matrix_norm_eunis-{lvl}', test_metrics[2])
    # Save logs
...
if MULTILABEL_CORRESPONDANCE_STRATEGY in ['soft_ml', 'ml']:
    ...
    for i in range(len(pred_df)):
        ...
        pred_df.loc[i, 'habitats_code'] = res[list(res.keys())[0]][floraveg_id]['all_labels']
        pred_df.loc[i, 'habitats_code_ID'] = res[list(res.keys())[0]][floraveg_id]['all_labels_enc_softml_str']
        for lvl in EUNIS_LVL:
            pred_df.loc[i, f'pred_label_lvl{lvl}'] = dataset_labels_table[res[lvl][floraveg_id]['all_preds']]
            pred_df.loc[i, f'pred_label_encoded_lvl{lvl}'] = res[lvl][floraveg_id]['all_preds']
            pred_df.loc[i, f'pred_confidence_lvl{lvl}'] = res[lvl][floraveg_id]['all_preds_confidence']
            pred_df.loc[i, f'valid_prediction_lvl{lvl}'] = str(int(pred_df.loc[i, f'pred_label_encoded_lvl{lvl}'])) in pred_df.loc[i, 'habitats_code_ID']
    for lvl in EUNIS_LVL:
